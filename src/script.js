...
window.updateHutang = async (id, tipe) => {
  const pesan = tipe === "tambah" ? "Tambah hutang sebesar:" : "Bayar hutang sebesar:";
  ...
  if (nominal && !isNaN(nominal)) {
    ...
    try {
      await updateDoc(docRef, { total_hutang: increment(nilai) });
...
window.lihatRiwayat = async (id, nama) => {
  ...
  if (querySnapshot.empty) {
    ...
    querySnapshot.forEach((doc) => {
      const t = doc.data();
      const tgl = t.tanggal.toDate().toLocaleDateString("id-ID", { day: "2-digit", month: "short", year: "numeric" });
      const jam = t.tanggal.toDate().toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit" });
...
window.filterPelanggan = () => {
  ...
  const kartu = document.getElementsByClassName('kartu-pelanggan');
  for (let i = 0; i < kartu.length; i++) {
    const nama = kartu[i].getElementsByTagName("h3")[0].innerText.toLowerCase();
    kartu[i].style.display = nama.includes(input) ? "" : "none";
  }
};

// --- KIRIM WA ---
window.kirimWA = (nama, total) => {
  const pesan = `Halo ${nama}, total hutang saat ini: Rp ${total.toLocaleString()}.`;
...
onSnapshot(query(colRef, orderBy("createdAt", "desc")), (snapshot) => {
  ...
  snapshot.forEach((doc) => {
    const data = doc.data();
    const id = doc.id;
    const warnaHutang = data.total_hutang > 0 ? "text-red-600" : "text-green-600";
    const borderHutang = data.total_hutang > 0 ? "border-red-500" : "border-green-500";
      ...
      <div class="kartu-pelanggan bg-white p-4 rounded-xl shadow-sm mb-3 border-l-4 ${borderHutang}">
        <div class="flex justify-between items-start">
          <div class="flex-1">
            ...
            <p class="text-xl font-mono font-bold ${warnaHutang}">Rp ${data.total_hutang.toLocaleString()}</p>
          </div>
          <div class="flex flex-col gap-2 min-w-[100px]">
            <button onclick="updateHutang('${id}', 'tambah')" class="bg-orange-500 text-white px-3 py-1.5 rounded-lg text-xs font-bold shadow-sm">+ Hutang</button>
            <button onclick="updateHutang('${id}', 'bayar')" class="bg-blue-600 text-white px-3 py-1.5 rounded-lg text-xs font-bold shadow-sm">Bayar</button>
            <button onclick="lihatRiwayat('${id}', '${data.nama}')" class="bg-gray-100 text-gray-700 px-3 py-1.5 rounded-lg text-xs font-bold border border-gray-300">Riwayat</button>
            <button onclick="kirimWA('${data.nama}', ${data.total_hutang})" class="bg-green-500 text-white px-3 py-1.5 rounded-lg text-xs font-bold shadow-sm">WA</button>
            <button onclick="hapusPelanggan('${id}', '${data.nama}')" class="mt-1 text-red-400 text-[10px] uppercase text-center">Hapus</button>
          </div>
        </div>
      </div>`;
  });
